import re
import json
import codecs
import logging
from urllib.parse import urlparse, parse_qs
from typing import Dict, Optional, List, Tuple
from functools import lru_cache

logger = logging.getLogger(__name__)

class LogParserError(Exception):
    """日志解析器异常类"""
    pass

class LogParser:
    def __init__(self, log_format_config):
        self.log_format_config = log_format_config
        self.fields = log_format_config.get('fields', {})
        self._pattern_cache = {}
        
        try:
            # 支持两种配置格式：字典和列表
            if isinstance(self.fields, dict):
                # 新格式：字典形式
                self.field_names = list(self.fields.keys())
                self.field_patterns = list(self.fields.values())
            else:
                # 旧格式：列表形式
                self.field_names = [field['name'] for field in self.fields]
                self.field_patterns = [field['regex'] for field in self.fields]
            
            self.regex_pattern = self._build_regex_pattern()
            self.regex = re.compile(self.regex_pattern)
            
            logger.info(f"日志解析器初始化成功，配置{len(self.field_names)}个字段")
            
        except Exception as e:
            logger.error(f"日志解析器初始化失败: {e}")
            raise LogParserError(f"解析器初始化失败: {e}")

    def _build_regex_pattern(self) -> str:
        """基于字段定义构建完整的正则表达式模式"""
        if not self.field_patterns:
            raise LogParserError("日志格式配置中未定义字段")
        
        # 拼接所有字段的正则表达式
        regex_parts = []
        for i, pattern in enumerate(self.field_patterns):
            try:
                # 确保模式有捕获组
                if '(' not in pattern:
                    pattern = f'({pattern})'
                # 验证正则表达式的有效性
                re.compile(pattern)
                regex_parts.append(pattern)
            except re.error as e:
                logger.error(f"字段{self.field_names[i]}的正则表达式无效: {pattern}, 错误: {e}")
                raise LogParserError(f"字段{self.field_names[i]}的正则表达式无效: {e}")
        
        # 组合成完整的正则表达式，添加适当的分隔符
        full_pattern = '\\s*'.join(regex_parts)
        return '^' + full_pattern + '\\s*$'

    def parse_log_line(self, line: str) -> Optional[Dict[str, str]]:
        """解析单条日志行并返回字典格式数据"""
        if not line or not line.strip():
            return None
            
        try:
            match = self.regex.match(line)
            if not match:
                # 如果完整模式匹配失败，尝试逐个匹配字段
                return self._partial_parse(line)

            # 提取匹配组并映射到字段名
            result = {}
            groups = match.groups()
            
            for i, field_name in enumerate(self.field_names):
                if i < len(groups):
                    result[field_name] = groups[i] if groups[i] is not None else ''
            
            # 特殊处理：从request_line或类似字段提取HTTP信息
            self._extract_http_info(result)
            
            # 解析JSON数据字段（如果存在）
            self._parse_json_fields(result)
            
            logger.debug(f"日志解析成功: {result.get('src_ip', 'N/A')}")
            return result
            
        except Exception as e:
            logger.warning(f"日志行解析失败: {str(e)[:100]}")
            return None

    def _partial_parse(self, line: str) -> Optional[Dict[str, str]]:
        """部分解析：逐个匹配字段"""
        result = {}
        remaining_line = line
        successful_matches = 0
        
        # 预编译正则表达式以提高性能
        compiled_patterns = self._get_compiled_patterns()
        
        for field_name, pattern in zip(self.field_names, compiled_patterns):
            try:
                match = pattern.search(remaining_line)
                if match:
                    result[field_name] = match.group(1)
                    successful_matches += 1
                    # 移除已匹配的部分
                    remaining_line = remaining_line[match.end():]
                else:
                    result[field_name] = ''
            except Exception as e:
                logger.debug(f"字段 {field_name} 解析失败: {e}")
                result[field_name] = ''
        
        # 如果至少有一半字段匹配成功，认为解析有效
        if successful_matches >= len(self.field_names) * 0.3:
            logger.debug(f"部分解析成功: {successful_matches}/{len(self.field_names)} 字段")
            return result
        else:
            logger.debug(f"部分解析失败: 仅 {successful_matches}/{len(self.field_names)} 字段匹配")
            return None

    @lru_cache(maxsize=None)
    def _get_compiled_patterns(self) -> List[re.Pattern]:
        """获取预编译的正则表达式模式（带缓存）"""
        compiled_patterns = []
        for pattern in self.field_patterns:
            # 确保模式有捕获组
            if '(' not in pattern:
                pattern = f'({pattern})'
            try:
                compiled_patterns.append(re.compile(pattern))
            except re.error as e:
                logger.error(f"编译正则表达式失败: {pattern}, 错误: {e}")
                # 使用一个匹配任意内容的模式作为备选
                compiled_patterns.append(re.compile(r'(.*)'))
        return compiled_patterns

    def _extract_http_info(self, result: Dict[str, str]):
        """从请求行提取HTTP信息"""
        # 尝试多个可能的字段名
        request_fields = ['request_line', 'request_method', 'request']
        
        for field in request_fields:
            if field in result and result[field]:
                try:
                    if field == 'request_line':
                        success = self._parse_request_line(result, result[field])
                        if success:
                            break
                    elif field == 'request_method':
                        result['method'] = result[field]
                    
                    # 解析URL参数
                    if 'url' in result:
                        self._parse_url_params(result)
                    
                except Exception as e:
                    logger.debug(f"HTTP信息提取失败 ({field}): {e}")

    def _parse_request_line(self, result: Dict[str, str], request_line: str) -> bool:
        """解析HTTP请求行"""
        try:
            # 完整请求行: "GET /path HTTP/1.1"
            parts = request_line.split()
            if len(parts) >= 3:
                result['method'] = parts[0]
                result['url'] = parts[1]
                result['protocol'] = parts[2]
                return True
            elif len(parts) >= 2:
                result['method'] = parts[0]
                result['url'] = parts[1]
                result['protocol'] = 'HTTP/1.1'
                return True
            else:
                logger.debug(f"请求行格式异常: {request_line}")
                return False
        except Exception as e:
            logger.debug(f"解析请求行失败: {e}")
            return False

    def _parse_url_params(self, result: Dict[str, str]):
        """解析URL参数"""
        try:
            url = result.get('url', '')
            if not url:
                return
                
            parsed_url = urlparse(url)
            result['query_params'] = parse_qs(parsed_url.query)
            result['path'] = parsed_url.path
            
            # 检查是否有可疑的URL模式
            if self._is_suspicious_url(url):
                result['suspicious_url'] = True
                
        except ValueError as e:
            logger.debug(f"URL格式无效: {e}")
            result['query_params'] = {}
        except Exception as e:
            logger.debug(f"URL参数解析失败: {e}")
            result['query_params'] = {}

    def _is_suspicious_url(self, url: str) -> bool:
        """检查URL是否包含可疑模式"""
        suspicious_patterns = [
            r'union\s+select',
            r'script\s*:',
            r'javascript\s*:',
            r'\.\./',
            r'<script',
            r'eval\s*\(',
            r'exec\s*\('
        ]
        
        url_lower = url.lower()
        for pattern in suspicious_patterns:
            if re.search(pattern, url_lower):
                return True
        return False

    def _parse_json_fields(self, result: Dict[str, str]):
        """解析JSON数据字段"""
        json_fields = ['json_data', 'data', 'payload']
        
        for field in json_fields:
            if field not in result or not result[field]:
                continue
                
            try:
                # 解码Unicode转义字符
                decoded_json = codecs.decode(result[field], 'unicode_escape')
                # 解析JSON
                json_data = json.loads(decoded_json)
                
                # 合并JSON数据到结果中
                for key, value in json_data.items():
                    if key not in result:  # 避免覆盖现有字段
                        result[key] = str(value)
                
                # 移除原始json字段以避免重复
                del result[field]
                logger.debug(f"成功解析JSON字段: {field}")
                break
                
            except (json.JSONDecodeError, UnicodeDecodeError) as e:
                logger.debug(f"JSON解析失败 ({field}): {e}")
            except Exception as e:
                logger.debug(f"处理JSON数据时出错 ({field}): {e}")

    def validate_config(self) -> Tuple[bool, List[str]]:
        """验证配置的有效性"""
        errors = []
        
        if not self.fields:
            errors.append("未定义日志字段")
        
        if not self.field_names:
            errors.append("字段名列表为空")
        
        if len(self.field_names) != len(self.field_patterns):
            errors.append("字段名与正则表达式数量不匹配")
        
        # 验证每个正则表达式
        for i, pattern in enumerate(self.field_patterns):
            try:
                re.compile(pattern)
            except re.error as e:
                errors.append(f"字段 {self.field_names[i]} 的正则表达式无效: {e}")
        
        return len(errors) == 0, errors

    def get_stats(self) -> Dict[str, int]:
        """获取解析统计信息"""
        return {
            'field_count': len(self.field_names),
            'pattern_cache_size': len(self._pattern_cache)
        }